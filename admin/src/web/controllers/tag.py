...
from src.core.tag_service import generate_slug, get_tag_by_slug, create_tag, list_tags
...
@tag_bp.route('/list', methods=['GET', 'POST'])
def add_tag():
    ...
    if not nombre:
        return render_template('tags.html', error="Ingrese un nombre para el tag.", tags=tags, pagination=pagination)
        
    #verifico que tenga entre 3 y 50 caracteres
    if len(nombre) < 3 or len(nombre) > 50:
        return render_template('tags.html', error="El nombre del tag debe tener entre 3 y 50 caracteres.", tags=tags, pagination=pagination)
    ...
    if existe_tag:
        return render_template('tags.html', error="El tag ya existe.", tags=tags, pagination=pagination)
    ...
    return render_template('tags.html', success = "El tag se creo correctamente.", tags=tags, pagination=pagination)
